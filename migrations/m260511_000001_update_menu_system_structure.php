...
class m260511_000001_update_menu_system_structure extends Migration
{
    ...
    {
        $menuSchema = $this->db->getTableSchema('master_menu');
        $pageSchema = $this->db->getTableSchema('master_page');
        
        // 1. Add type column to master_menu (group, page, route) - if not exists
        if (!isset($menuSchema->columns['type'])) {
            $this->execute("ALTER TABLE master_menu ADD COLUMN type ENUM('group', 'page', 'route') DEFAULT 'page' AFTER parent_id");
        }
        
        // 2. Add order column (alias for sort_order but more standard) - if not exists
        if (!isset($menuSchema->columns['order'])) {
        ...
        // 3. Add slug to master_page - if not exists
        if (!isset($pageSchema->columns['slug'])) {
            $this->execute("ALTER TABLE master_page ADD COLUMN slug VARCHAR(100) DEFAULT NULL AFTER title");
        }
        
        // 4. Rename layout_type to layout for consistency - if not exists
        if (!isset($pageSchema->columns['layout'])) {
            $this->execute("ALTER TABLE master_page ADD COLUMN layout VARCHAR(50) DEFAULT 'default' AFTER slug");
        }
        
        // 5. Create page_forms table (linking pages to forms with order) - if not exists
        if ($this->db->getTableSchema('page_forms') === null) {
